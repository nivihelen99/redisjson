...
TEST_F(JSONModifierTest, ApplyPatch) {
    ...
    modifier.apply_patch(test_doc, patch_doc); // Pass the parsed patch document
    EXPECT_EQ(test_doc["name"], "RedisJSON++ NextGen");
    EXPECT_EQ(test_doc["details"]["year"], 2025);
}

TEST_F(JSONModifierTest, Diff) {
    json original_doc = test_doc; // nlohmann::json supports copy assignment/constructor
    json modified_doc = test_doc; // Create a copy
    modified_doc["version"] = 1.1;
    modified_doc["features"].push_back("extra");

    json diff_patch = modifier.diff(original_doc, modified_doc);
    ...
    json temp_doc = original_doc.patch(diff_patch); // Apply the diff to the original
